...
- AI website generation from text descriptions — two-stage pipeline: Stage 1 (GPT-4o-mini) generates structured bilingual content JSON (spec with Arabic primary + English "en" sub-object), Stage 2 (TypeScript) builds premium guaranteed HTML with correct CSS Grid, RTL, lightbox, FAQ, counters, and built-in AR/EN language toggle. Images injected by TypeScript (not AI) via Unsplash/Pexels APIs. Special categories (wedding/freelancer/event) use legacy AI HTML generation.
...
- Download Center (/download-center): dedicated page for all download + delete operations; shows projects in grid with Download ZIP (triggers learning signal) + Delete + Edit + Preview; Download is ONLY available here, not in editor toolbar
- Dashboard project cards: dropdown menu redirects to Download Center for downloads; sidebar "مركز التحميل" button navigates to /download-center
