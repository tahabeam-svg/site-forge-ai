...
export default function EditorPage() {
  ...
  const publishMutation = useMutation({
    ...
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: ["/api/projects", projectId] });
      const vercelUrl = data?.publishedUrl;
      if (vercelUrl) {
        toast({
          title: lang === "ar" ? "✅ تم النشر على Vercel" : "✅ Published on Vercel",
          description: (
            <div className="flex flex-col gap-2 mt-1">
              <a href={vercelUrl} target="_blank" rel="noopener noreferrer"
                className="text-emerald-600 underline text-xs break-all font-mono">{vercelUrl}</a>
              <button
                className="text-xs bg-emerald-50 border border-emerald-200 text-emerald-700 rounded px-2 py-1 hover:bg-emerald-100 w-fit"
                  ...
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => window.open((project as any).publishedUrl, "_blank")}
                        data-testid="button-open-vercel-url"
                        className="hover:border-emerald-400 hover:text-emerald-600 font-mono text-xs max-w-[160px] truncate gap-1.5"
                      >
                        <ExternalLink className="w-3.5 h-3.5 shrink-0" />
                        <span className="truncate">{((project as any).publishedUrl as string).replace("https://", "")}</span>
                      </Button>
                    </TooltipTrigger>
                    <TooltipContent side="bottom">{lang === "ar" ? "فتح الموقع المنشور" : "Open published site"}</TooltipContent>
                  </Tooltip>
                  {/* Copy URL */}
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Button
                        variant="outline"
                        size="icon"
                        className="h-8 w-8 hover:border-violet-400 hover:text-violet-600"
                        data-testid="button-copy-url"
                        onClick={() => {
                          navigator.clipboard.writeText((project as any).publishedUrl);
                          toast({ title: lang === "ar" ? "تم نسخ الرابط ✓" : "URL copied ✓", duration: 2000 });
                        }}
                      >
                        <Copy className="w-3.5 h-3.5" />
                      </Button>
                    </TooltipTrigger>
                    <TooltipContent side="bottom">{lang === "ar" ? "نسخ رابط الموقع" : "Copy site URL"}</TooltipContent>
                  </Tooltip>
                  {/* WhatsApp share */}
                  <Tooltip>
                    <TooltipTrigger asChild>
